...
const CommonModal = ({ onToggle, modalName, modalId, size }: VariousModal) => {
  const [modalSize, setModalSize] = useState<string>('max-w-md');

  const getModalSize = () => {
    if (size === 'sm') {
      setModalSize('max-w-md');
    } else if (size === 'md') {
      setModalSize('max-w-lg');
    } else if (size === 'lg') {
      setModalSize('max-w-4xl');
    } else if (size === 'xl') {
      setModalSize('max-w-7xl');
    } else {
      setModalSize('w-screen h-screen');
    }
  };
  ...
  return (
    <div
      id={modalId}
      tabIndex={-1}
      className={`fixed flex justify-center items-center z-50 inset-0 p-4 ${
        size === 'full' ? 'w-screen h-screen' : ''
      } overflow-x-hidden overflow-y-auto md:inset-0 max-h-full`}
    >
      <div className={`relative ${modalSize} max-h-full`}>
        <div className="relative bg-white h-full rounded-lg shadow dark:bg-gray-700">
          <div className="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
            <h3 className="text-xl font-medium text-gray-900 dark:text-white">우리동네소식</h3>
            <button
              type="button"
              className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
              data-modal-hide="small-modal"
              onClick={() => {
                onToggle(modalName ? modalName : '');
              }}
            >
